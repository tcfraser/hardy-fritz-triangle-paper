...
\begin{document}
    ...
    \end{definition}
    \begin{definition}
        A set of \term{causal parameters} for a particular causal structure $\graph$ is the specification of a conditional distribution for every node $v \in \nodes$ given it's parents in $\graph$.
        \[ \bc{\prob[v \mid \Pa[\graph]{v}] \mid v \in \nodes} \]
    \end{definition}
    ...
    \begin{definition}
        A marginal distribution $\prob[V]$ is \term{compatible} with a causal structure $\graph$ (where is assumed that $V \subseteq \nodes_O$) if there exists a \textit{choice} of causal parameters $\bc{\prob[v \mid \Pa[\graph]{v}] \mid v \in \nodes}$ such that $\prob[V]$ can be obtained from the following operations: First obtain the
    \end{definition}
    Given a causal structure $\graph = \br{\nodes, \edges}$ together with set of marginal distributions over various subsets of $\nodes$,
